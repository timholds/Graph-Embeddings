...
d3.json(input_net, function(error, graph) {
  ...
  var textLabels = text
                   .attr("x", width/2-200)
                   .attr("y", 30)
                   .text("There are 0 authors in 0 communities.")
                   .attr("z-index", -1)
      ...
      if (typeof seq != 'undefined' && seq.length > 0) {
        ...
        if ( nodes_hash.hasOwnProperty(nodeinfo[0]) ) {
          ...
          svg.select( "#" + node.id )
             ...
             .style("fill", color(Math.sqrt(node.value)*3));
        }
        else {
        ...
        } //hashtag.charAt(0).toUpperCase() + hashtag.slice(1) num_user
        
        // Update text
        svg.select('#viz-title')
           .text("Quanta: "+
                 "Cited by "+
                 nodes.length+" communities by the year "+
                 cur_date_string);
      }
    }, speed);
