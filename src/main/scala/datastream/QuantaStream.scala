...
object QuantaStream {
  ...
  case class Quanta(title: String, lang: String, year: Int, references: List[String],
    `abstract`: String, url: List[String], id: String, fos: List[String])
  ...
  def fileToStream(dataFile: File): Stream[IO, Quanta] = {
    ...
    val popularityStream: Stream[IO, Quanta] = parsedStream.through(decoder[IO, Quanta])
  ...
  val dataFiles: List[File] = getListOfFiles(dataFolder).filter(f => f.getName.endsWith(".txt"))

  import scala.concurrent.ExecutionContext.Implicits.global
  val quantaStream: Stream[IO, Quanta] = dataFiles.map(fileToStream).reduce((a,b)=>a.merge(b))
